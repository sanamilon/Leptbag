...
class gen6DofInsect{
	...
	contractileElement thorax_legFrontLeft;
	contractileElement thorax_legFrontRight;
	contractileElement thorax_legBackLeft;
	contractileElement thorax_legBackRight;
	...
	void spawn(float x, float y, float z){
		...
		legFrontLeft = getCubeshape().generate(paramWrap(
					param("position", createVec3(x+0.8, y, z-1)),
					param("scale", createVec3(0.2, 0.1, 0.5)),
					param("rotation", createQuat(1, 0, 0, 0)),
					param("mass", 0.2f)));
		...
		gen6Dof_thorax_legFrontLeft   = generic6DofConstraint_create(thorax , legFrontLeft , createVec3( 0.8, 0, -1), createVec3(   0, 0,  0.5), true);
		gen6Dof_thorax_legFrontRight  = generic6DofConstraint_create(thorax , legFrontRight, createVec3( 0.8, 0,  1), createVec3(   0, 0, -0.5), false);
		gen6Dof_thorax_legBackLeft	  = generic6DofConstraint_create(thorax , legBackLeft  , createVec3(-0.8, 0, -1), createVec3(   0, 0,  0.5), true);
		gen6Dof_thorax_legBackRight	  = generic6DofConstraint_create(thorax , legBackRight , createVec3(-0.8, 0,  1), createVec3(   0, 0, -0.5), false);

		// リストにぶち込んでやるぜ的なやーつでこの辺綺麗にしたい。
		// というかデフォルトは全部0なのでは…？
		gen6Dof_thorax_legFrontLeft.setAngularLowerLimit( createVec3(   0,    -PI/2+0.1,    0.2));
		gen6Dof_thorax_legFrontLeft.setAngularUpperLimit( createVec3(   0,    PI/2-0.1, 0.2));
		...
		gen6Dof_thorax_legFrontRight.setAngularLowerLimit( createVec3(   0,    -PI/2+0.1,    0.2));
		gen6Dof_thorax_legFrontRight.setAngularUpperLimit( createVec3(   0,     PI/2-0.1, 0.2));
		...
		gen6Dof_thorax_legBackLeft.setAngularLowerLimit( createVec3(   0,    -PI/2+0.1,    0.2));
		gen6Dof_thorax_legBackLeft.setAngularUpperLimit( createVec3(   0,    PI/2-0.1, 0.2));
		...
		gen6Dof_thorax_legBackRight.setAngularLowerLimit( createVec3(   0,     -PI/2+0.1,    0.2));
		gen6Dof_thorax_legBackRight.setAngularUpperLimit( createVec3(   0,    PI/2-0.1, 0.2));
